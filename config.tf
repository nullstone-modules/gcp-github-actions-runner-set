locals {
  ...
  // Fall back to default github actions image if we don't have a repository url
  runner_image_repo = coalesce(local.repository_url, "ghcr.io/actions/actions-runner")
  runner_image_tag  = "${local.runner_image_repo}:${local.app_version}"
